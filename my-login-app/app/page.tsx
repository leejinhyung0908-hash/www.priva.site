...
function LoginContent() {
  ...
  const handleKakaoLogin = async () => {
    const loginUrl = `http://localhost:8080/kakao/login?t=${Date.now()}`;
    ...
    try {
      await fetch('/api/log/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'Gateway 카카오 연결', url: loginUrl }),
      });
    } catch { }
  ...
  const handleNaverLogin = async () => {
    const loginUrl = `${process.env.NEXT_PUBLIC_API_URL}/naver/login?t=${Date.now()}`;
    try {
      await fetch('/api/log/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'Gateway 네이버 연결', url: loginUrl }),
      });
    } catch {}
    window.location.href = loginUrl;
  };
  ...
  const handleGoogleLogin = async () => {
    const loginUrl = `${process.env.NEXT_PUBLIC_API_URL}/google/login?t=${Date.now()}`;
    try {
      await fetch('/api/log/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'Gateway 구글 연결', url: loginUrl }),
      });
    } catch {}
    window.location.href = loginUrl;
};
